...
const Chat = () => {
  const easterEggReply = {
    heroText:
      "أول Easter Egg موجود في قسم الـ Hero عند الضغط على جملة معينة، الثاني يظهر عند هز الموبايل، الثالث يتفاعل عند تشغيل الأغنية، والرابع عند البحث عن input مخفي في الموقع. استمر في الاستكشاف لمزيد من المفاجآت!",
  };
        ...
        >
          <div className="h-[270px] overflow-auto max-sm:h-[330px]">
            {message.map((msg, index) => (
              <div
                key={index}
                className={`flex w-fit gap-3 mb-2 rounded-2xl ${
                  msg.role === "assistant"
